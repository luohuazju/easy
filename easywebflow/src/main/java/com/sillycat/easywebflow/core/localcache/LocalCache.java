...
public class LocalCache<K, V> implements Cache<K, V> {
	 ...
	 */
	protected Map<K, LocalCache.CacheObject<V>> map;

	/**
	 * Linked list to maintain order that cache objects are accessed in, most
	 * used to least used.
	 */
	protected com.sillycat.easywebflow.core.localcache.LinkedList lastAccessedList;

	/**
	 * Linked list to maintain time that cache objects were initially added to
	 * the cache, most recently added to oldest added.
	 */
	protected com.sillycat.easywebflow.core.localcache.LinkedList ageList;
	...
	/**
	 * Create a new default cache and specify the maximum size of for the cache
	 * in bytes, and the maximum lifetime of objects.
	 * 
	 * @param name
	...
	public LocalCache(String name, long maxSize, long maxLifetime) {
		...
		lastAccessedList = new com.sillycat.easywebflow.core.localcache.LinkedList();
		ageList = new com.sillycat.easywebflow.core.localcache.LinkedList();
	}
	...
	public synchronized V put(K key, V value) {
		...
		LocalCache.CacheObject<V> cacheObject = new LocalCache.CacheObject<V>(
				value, objectSize);
	...
	public synchronized V get(Object key) {
		...
		LocalCache.CacheObject<V> cacheObject = map.get(key);
		if (cacheObject == null) {
	...
	public Collection<V> values() {
		...
		// maximum defined age.
		deleteExpiredEntries();
		return new LocalCache.CacheObjectCollection(map.values());
	}
	...
	private final class CacheObjectCollection<V> implements Collection<V> {
		private Collection<LocalCache.CacheObject<V>> cachedObjects;

		private CacheObjectCollection(
				Collection<LocalCache.CacheObject<V>> cachedObjects) {
		...
		public Iterator<V> iterator() {
			return new Iterator<V>() {
				private final Iterator<LocalCache.CacheObject<V>> it = cachedObjects
						.iterator();
				...
				public V next() {
					if (it.hasNext()) {
						LocalCache.CacheObject<V> object = it.next();
						if (object == null) {
							return null;
						} else {
	...
	public Set<Entry<K, V>> entrySet() {
		...
		synchronized (this) {
			...
			for (final Entry<K, LocalCache.CacheObject<V>> entry : map
					.entrySet()) {
	...
	protected final void cullCache() {
		...
		if (cacheSize >= desiredSize) {
			...
			if (cacheSize > desiredSize) {
				...
				Log.warn("Cache " + name + " was full, shrinked to 90% in " + t
						+ "ms.");
			}
		}
	}
